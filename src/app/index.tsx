...
import { dbSerive } from "../lib";

const Home = () => {
  const [samples, setSamples] = useState<{ text: string; id: string }[]>([]);
  useEffect(() => {
    const subscribe = dbSerive.collection("sample").onSnapshot((snap) => {
  ...
  //! database 지역,날씨,기기,인터넷속도 등에 영향받음
  //? async await
  const onDelete = async (id: string) => {
    const ref = dbSerive.collection("sample").doc(id);
    //! trycatch로 감싸서 에러 핸들링하기
    try {
      await ref.delete();
    } catch (error: any) {
      console.log(error);
  ...
  const onAdd = async (newText: string) => {
    try {
      const ref = dbSerive.collection("sample");
  ...
  const onEdit = async (id: string) => {
    try {
      const ref = dbSerive.collection("sample");
      await ref.doc(id).update({ text: "리액트배우기" });
      alert("수정되었습니다.");
    } catch (error: any) {
      alert(error.message);
    }
  ...
  return (
    <Container.Col>
      ...
      <Button.Opacity onClick={() => onAdd("새로운할일2")}>추가</Button.Opacity>
      <ul>
        {samples.map((sample) => (
          <li key={sample.id}>
    ...
    <Container.Row className="w-full h-screen justify-center items-center">
      <Container.Col className="gap-y-5 max-w-90 p-5">
        <Typo.H1 className="text-center relative">
          ...
          <TiHeart className="absolute bottom-25 left-80 rotate-30 text-pink-500 " />
        </Typo.H1>
